...
```{r}
xt = xchisq.test(Benford$Invoices, Benford$BenfordP)
p.value = xt$p.value; p.value
xsq = xt$statistic; xsq
df = xt$parameter; df
```
We find that the p-value $\approx$ `r p.value` using the upper tail beyond `r xsq` of a chi-square distribution with `r df` degrees of freedom.
This is not a small p-value, so we do not have enough evidence that the first digit of invoices do not follow Benford's law.
